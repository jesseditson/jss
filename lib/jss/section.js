...
Section.prototype.modifiers = function(){
	...
	commentSections.split("\n").forEach(function(value){
		if(trim(value).length){
			var indent = value.match(/^\s*/)[0].length;
		
			if(typeof last_indent === 'number' && indent > last_indent){
				/// if this line is indented, append the text to the last modifier's description
				modifiers[modifiers.length - 1].description += value.replace(/\s+/g," ");
			} else {
				var info = value.split(/\s? - \s?/),
					modifier = info[0],
					desc = info[1];
